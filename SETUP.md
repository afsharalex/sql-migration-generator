...
* [`src/sql_migration_generator/`](src/sql_migration_generator/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/sql_migration_generator/core.cljs): contains the SPA entry point, `init`
* [`test/sql_migration_generator/`](test/sql_migration_generator/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `sql-migration-generator` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/sql_migration_generator/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns sql-migration-generator.example
  (:require [sql-migration-generator.config :as config])
